...
from main import get_local_map_boundaries
import os
import cv2
from door_list_save import door_list_dict
...
from FFD_frontier import FFD_frontier_detection
import sys
import matplotlib
...
scene_name_ = 'Nicut'
try:
    prior_door_list_store = door_list_dict[scene_name_]
except:
    prior_door_list_store =[]

for content in prior_door_list_store:
...
from door_list_save import Cantwell_graph
...
def main():
    ...
    for scene_idx in tqdm(range(3)):
        ...
        prior_door_list = prior_door_list_store.copy()
        # print('pp_list {}'.format(prior_door_list))
        # print('start_next_scene')
        frontier_detector = FFD_frontier_detection(args.map_size_cm // args.map_resolution)#Frontier_detection(args.map_size_cm // args.map_resolution)
        ...
        def take_action(action, locs, first_flag=True, long_term_goal=None, room_search_flag = False):
            ...
            if action != 4:
                ...
                if done:  # means this round should be over  action_count == args.max_episode_length - 1
                    save_path = os.path.join('/home/airs/Downloads/ANS/Neural-SLAM/8m_add_exp/efficiency/FFD',
                                             scene_name,
                                             '{}_{}_cov.npy'.format(scene_name, scene_idx))  # scene_idx
                    save_path_time = os.path.join('/home/airs/Downloads/ANS/Neural-SLAM/8m_add_exp/efficiency/FFD',
                                                  scene_name,
                                                  '{}_{}_time.npy'.format(scene_name, scene_idx))  # scene_idx
                    save_path_area = os.path.join('/home/airs/Downloads/ANS/Neural-SLAM/8m_add_exp/efficiency/FFD',
                                                  scene_name,
                                                  '{}_{}_area.npy'.format(scene_name, scene_idx))  # scene_idx
                    file_path = os.path.join('/home/airs/Downloads/ANS/Neural-SLAM/8m_add_exp/efficiency/FFD',
                                             scene_name)
                    #try:
                ...
                f_list, info_gain_list, show_map, room_exp_list, door_grid = frontier_detector.frontier_detection(np.rint(gt_map), np.rint(gt_exp),laser_list,np.array(current_loc))
                #step_list.append(time()-f_start_time)
                print('frontier is {}'.format(f_list))
        ...
        def room_searching(locs, stg, long_term_goal, first_flag, whether_returning):
            if long_term_goal:
                ...
                step_limit = 100  # 20 for my method
                step_count = 0
